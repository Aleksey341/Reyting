-- ⚡ КРИТИЧНО: Заполнить ФИО глав МО
-- Выполнить на production Amvera БД сейчас!
-- Копировать весь блок и вставить в psql или SQL консоль

BEGIN;

-- Убедиться что колонка существует
ALTER TABLE dim_mo ADD COLUMN IF NOT EXISTS leader_name VARCHAR(255);

-- ЗАПОЛНИТЬ 20 РЕАЛЬНЫХ ФИО ГЛАВ МО ЛИПЕЦКОЙ ОБЛАСТИ
